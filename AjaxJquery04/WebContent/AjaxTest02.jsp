	...
	$(function()
	{
		$("#sendBtn").click(function()
		{
			...
			$.ajax(
			{
				...
				, data:params					// 요청페이지(AjaxTest01_ok.jsp)에서 수신하게 되는 데이터
			...
			*/
			$.ajax(
			{
				...
				, dataType:"xml"				// 저쪽에서 내가 받게 되는 것      check
				//, async:false					// false = 동기 -> ajax 쓸 이유가 없음
				, success:function(args)
				{
					var out = "";
					out = ">> 게시물 갯수 : " + $(args).find("total_datacount").text() + "<br>";// 저 태그의 텍스트(5)를 찾아라
					
					$(args).find("comment").each(function()	//each : 각각 접근하게 함
					{
						var item = $(this);
